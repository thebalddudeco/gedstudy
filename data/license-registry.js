window.GEDData = window.GEDData || {};

window.GEDData.licenseRegistry = {
  sellerName: "https://thebalddude.gumroad.com",
  productName: "GED Prep Hub",
  supportEmail: "info@thebalddude.co",
  instructions:
    "Use the same email address used at checkout and the Gumroad license key assigned to your purchase.",
  activeLicenses: [
    // Paste objects from license-helper.html here.
    // Example:
    // { id: "owner-test-license", hash: "paste-generated-hash-here", note: "Owner test access" }
  ],
  revokedLicenseHashes: [
    // Paste hashes here to revoke access later.
    // A revoked hash will fail even if it still appears in activeLicenses.
  ]
};
